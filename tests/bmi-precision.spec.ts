...
test.describe('BMI Calculator - Acceptance Criteria #3', () => {
  ...
  for (const testCase of testCases) {
    test(`should use full precision for calculations and round only for display (height ${testCase.height} cm, weight ${testCase.weight} kg)`, { tag: ['@regression'] }, async ({ page }) => {
      const bmiPage = new BmiCalculatorPage(page);
